...
def group_by_centroid(restaurants, centroids):
    ...
    """
    return group_by_first([ [find_closest(restaurant_location(item), centroids), item] for item\
            in restaurants])
...
def find_centroid(restaurants):
    ...
    for restaurant in restaurants:
        location_list.append(restaurant_location(restaurant))
    ...
    for x, y in  location_list:
        laditude.append(x)
        longitude.append(y)
    return [mean(laditude), mean(longitude)]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(i) for i in restaurant_cluster]
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    for i in range(len(xs)):
        temp.append(pow(xs[i]- mean(xs), 2))
    Sxx = sum(temp)

    temp2 = []
    for i in range(len(ys)):
        temp2.append(pow(ys[i]- mean(ys), 2))
    Syy = sum(temp2)
    temp3 = []
    for i in range(len(ys)):
        temp3.append((xs[i] - mean(xs))*(ys[i] - mean(ys)))
    Sxy = sum(temp3)
    b = Sxy/Sxx
    a, r_squared =mean(ys) -b*mean(xs), pow(Sxy, 2)/(Sxx*Syy)  # REPLACE THIS LINE WITH YOUR SOLUTION
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for fn in feature_fns:
        temp.append(find_predictor(user, reviewed, fn))
    temp = dict(temp)
    ...
    for name in restaurants:
        for restaurant in restaurants.values():
            if name in user_reviewed_restaurants(user, restaurants):
                temp.append([name, user_rating(user, name)])
            else:
                temp.append([name, predictor(restaurant)])
    return dict(temp)
