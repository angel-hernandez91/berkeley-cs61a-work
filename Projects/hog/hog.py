#Angel Hernandez, cs61a - bvu
#Leanne Lee, cs61a - afr
...
def prime_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    otherwise.
    """
    # margin_test stores the value of that would result from the free
    # bacon rule being used
    margin_test = max((opponent_score // 10), (opponent_score % 10)) +1
    # test_score will be used to check if free bacon will result in a prime
...
def final_strategy(score, opponent_score):
    ...
    margin_test = max((opponent_score // 10), (opponent_score % 10)) +1
    test_score = score + opponent_score + margin_test
    hog_max = max((opponent_score//10), (opponent_score %10)) + 1
    marg = opponent_score -1
   

    if hog_max >= margin:
        return 0
    if score > opponent_score and (100 - score + hog_max) < 15 :
        return 2
    elif score > opponent_score and (110 - score - marg) > 80:        
        return 4
    elif score > opponent_score and (110 - score ) < 20 and (110 - score > 16):
        return 3
    elif score < opponent_score and (100 - opponent_score - hog_max) < 70:
        return 7
        ...
        if margin_test >= margin:
            ...
            return 4
    #default roll of 5 die if none of the above conditions are met. 
    else:
        return 5
