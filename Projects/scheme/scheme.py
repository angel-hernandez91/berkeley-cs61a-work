...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    try:
        ...
        raise SchemeError("Not a primitive.")
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Too many or too few arugments to function call")
        else:
            if len(formals) > 0 and len(vals) > 0:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lambda_expression = Pair('lambda', args)
        env.define(target.first, scheme_eval(lambda_expression, env))
        return target.first 
    else:
...
def do_and_form(expressions, env):
    ...
    for exp in expressions:
        if exp == expressions[len(expressions) -1]:
            total = scheme_eval(exp, env, True)
        elif not (exp == expressions[len(expressions) -1]):
            total = scheme_eval(exp, env)
        if scheme_false(total):
            return False
    return total
...
def make_let_frame(bindings, env):
    ...
    procedures, args = nil, nil
    for i in range(len(bindings)):
        check_form(bindings[i], 1, 2)
        if not scheme_symbolp(bindings[i].first):
            raise SchemeError("{0} is not a symbol.".format(bindings[i].first))
        else:
            procedures = Pair(bindings[i].first, procedures)
            args = Pair(scheme_eval(bindings[i].second.first, env), args)
            check_formals(procedures)
    return env.make_child_frame(procedures, args)
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError(str(formals) +' not a well-formed list of symbols')  
    args = []
    for formal in formals:
        if formal in args:
            raise SchemeError('repeated symbols: '+ str(formals))
        elif not scheme_symbolp(formal):
            raise SchemeError('Non-symbol: '+str(formal))
        args.append(formal)
...
def do_mu_form(expressions, env):
    ...
    if len(expressions) < 2:
        mu_form = Pair('begin', expressions.second)
    else:
        mu_form = expressions.second
