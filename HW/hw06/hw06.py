    ...
    def weight(self):
        ...
        def weight_sum(obj):
            if isinstance(obj, Weight):
                return obj.weight
            elif isinstance(obj, Mobile):
                return weight_sum(obj.left) + weight_sum(obj.right)
            else:
                return weight_sum(obj.contents)
        return weight_sum(self)

    def is_balanced(self):
        """True if and only if the mobile is balanced."""
        left_torque = self.left.torque*self.left.length
        right_torque = self.right.torque*self.left.length
        if left_torque == right_torque and self.left.contents.is_balanced() \
            and self.right.contents.is_balanced():
            return True
        else:
            return False
